...
public class OrderCodeTools {
    ...
    public static final String REFUND_CODE = "TK";
    ...
    public static String createRefundCode(String spotCode, int number){
        //获取当前日期
        Calendar calendar = Calendar.getInstance();
        String year = calendar.get(Calendar.YEAR)+"";
        String month = (calendar.get(Calendar.MONTH)+1) < 10 ? "0"+(calendar.get(Calendar.MONTH)+1) : (calendar.get(Calendar.MONTH)+1)+"";
        String day = calendar.get(Calendar.DATE) < 10 ? "0"+calendar.get(Calendar.DATE) : calendar.get(Calendar.DATE)+"";
        StringBuilder sb = new StringBuilder();
        sb.append(REFUND_CODE);
        sb.append(spotCode);
        sb.append(year);
        sb.append(month);
        sb.append(day);
        sb.append(String.format(TWO_DIGIT, number));
        return sb.toString();
    }
}
