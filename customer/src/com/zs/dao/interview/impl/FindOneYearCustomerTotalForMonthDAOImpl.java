 ...
 */
@Service("findOneYearCustomerTotalForMonthDAO")
public class FindOneYearCustomerTotalForMonthDAOImpl extends BaseQueryDao
    ...
    public List<Object[]> findListByWhere(Map<String, String> paramsMap, Map<String, Boolean> sortMap) {
        ...
        String sql = "select ttt.month, ifnull(tt.customerCount, 0)  from (";
        sql += "SELECT year, month, count(*) customerCount FROM (";
        sql += "SELECT DISTINCT YEAR (i.create_time) year, MONTH (i.create_time) MONTH, i.customer_id ";
        sql += "FROM interview i ";
        sql += "WHERE i.create_time >= ? AND i.create_time <= ? )t ";
